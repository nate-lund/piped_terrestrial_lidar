#================================ Clipe .las files by AOI ================================
# This is a WIP, mostly needs to wait until I can georeference everything

#================================ Tile .las ================================

#' [Test code]
# las_path =  "H:/_terrestrial-lidar_working/_working/_full_clouds/2026-08-06_WD_o005.laz"

# output_folder = "H:/_terrestrial-lidar_working/_working/_tiles"

tile_cloud = function(las_path, output_folder, tile_size){
  ...
  opt_chunk_size(ctg) = tile_size # Tile size in meters 
  ...
  opt_laz_compression(ctg) = FALSE # If FALSE, retile will produce .las instead of .laz
  
  # catalog_retile() processes and writes tile-by-tile, never loading cloud into
  # memory. Takes some time, shows progress in a plot. 
  print(paste0("Re-tileing: ", base_name))
  tiled_ctg <- catalog_retile(ctg)
  
  # Create .lax files for each tile
  lapply(tiled_ctg$filename, rlas::writelax)
...
#' [Test code]
# tiles = tar_read(rough_classification)

# output_folder = "H:/_terrestrial-lidar_working/_working/_ground-only"

filter_ground = function(tiles, output_folder){

  # Grab original .las file name
  base_name <- sub("_[-0-9]+_[-0-9]+_classified$", "", tools::file_path_sans_ext(basename(tiles[1])))
